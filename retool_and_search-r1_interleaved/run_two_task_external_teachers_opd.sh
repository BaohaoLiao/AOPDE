...
RETOOL_MODEL_DIR=${RETOOL_MODEL_DIR:-/mount/coreai-genai-pvc/baliao/experiments/agentic_opd/00_single_env/retool}
SEARCH_R1_MODEL_DIR=${SEARCH_R1_MODEL_DIR:-/mount/coreai-genai-pvc/baliao/experiments/agentic_opd/00_single_env/search_r1}
BOTH_MODEL_DIR=${BOTH_MODEL_DIR:-/mount/coreai-genai-pvc/baliao/experiments/agentic_opd/00_single_env/both}
...
SAVE_DIR=${SAVE_DIR:-${BOTH_MODEL_DIR}/Qwen3-4B-Instruct-2507-SFT-RL-to-Qwen3-4B-Instruct-2507-SFT-interleaved-two-task}
...
# One rollout batch contains one task. This order alternates batches by task.
# Repeating a task changes the rollout-step ratio, e.g. retool,search-r1,search-r1.
INTERLEAVED_TASK_ORDER=${INTERLEAVED_TASK_ORDER:-retool,search-r1}
...
if [ "${START_RETOOL_TEACHER}" = "1" ]; then
   RETOOL_LAUNCH_INFO=$(
      launch_teacher \
         ...
         "${RETOOL_TEACHER_MAX_TOTAL_TOKENS:-16384}" \
...
if [ "${START_SEARCH_R1_TEACHER}" = "1" ]; then
   SEARCH_R1_LAUNCH_INFO=$(
      launch_teacher \
         ...
         "${SEARCH_R1_TEACHER_MEM_FRACTION_STATIC:-0.80}" \
         "${SEARCH_R1_TEACHER_MAX_RUNNING_REQUESTS:-128}" \
         "${SEARCH_R1_TEACHER_SCHEDULE_CONSERVATIVENESS:-0.5}" \
         "${SEARCH_R1_TEACHER_MAX_TOTAL_TOKENS:-16384}" \
...
WANDB_GROUP=${WANDB_GROUP:-Both_Qwen3-4B-Instruct-2507-SFT-RL-to-Qwen3-4B-Instruct-2507-SFT-interleaved-two-task}
...
export MULTITEACHER_OPD_TEACHER_MAX_INFLIGHT=${MULTITEACHER_OPD_TEACHER_MAX_INFLIGHT:-8}
export MULTITEACHER_OPD_TEACHER_REQUEST_TIMEOUT=${MULTITEACHER_OPD_TEACHER_REQUEST_TIMEOUT:-1800}
export MULTITEACHER_OPD_TEACHER_CONNECT_TIMEOUT=${MULTITEACHER_OPD_TEACHER_CONNECT_TIMEOUT:-60}
export MULTITEACHER_OPD_TEACHER_SOCK_READ_TIMEOUT=${MULTITEACHER_OPD_TEACHER_SOCK_READ_TIMEOUT:-900}
export MULTITEACHER_OPD_TEACHER_TOTAL_BUDGET=${MULTITEACHER_OPD_TEACHER_TOTAL_BUDGET:-2400}
export MULTITEACHER_OPD_TEACHER_MAX_RETRIES=${MULTITEACHER_OPD_TEACHER_MAX_RETRIES:-3}
